...
int main0()
{
    ...
    // ------------------------------
    glfwInit();//初始化GLFW
    glfwWindowHint(GLFW_CONTEXT_VERSION_MAJOR, 3); //penGL版本是3.3,主版本号(Major)和次版本号(Minor)都设为3
...
int main1()
{
    ...
    // glad: load all OpenGL function pointers
    // ---------------------------------------
    if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)) //获取函数指针地址
    {
        std::cout << "Failed to initialize GLAD" << std::endl;
        return -1;
    }
    ...
    glGenVertexArrays(1, &VAO); //必要的顶点数组
    glGenBuffers(1, &VBO);
    glGenBuffers(1, &EBO);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, 0);//安全解绑，好习惯
